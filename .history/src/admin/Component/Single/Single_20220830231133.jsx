...
function Single() {
    ...
    const { id } = useParams();
    const users = userRows.filter((userRow, index) => {
        return userRow._id === id;
    });
            ...
            <div className={cx('singleContainer')}>
                <div className={cx('top')}>
                    <div className={cx('left')}>
                        ...
                        {users.map((userRow) => {
                            return (
                                <div className={cx('item')} key={userRow._id}>
                                    <img src={userRow.img} alt="" className={cx('itemImg')} />
                                    {console.log(userRow._id)}
                                    <div className={cx('details')}>
                                        <div className={cx('detailItem')}>
                                            <h1 className={cx('itemTitle')}>{userRow.username}</h1>
